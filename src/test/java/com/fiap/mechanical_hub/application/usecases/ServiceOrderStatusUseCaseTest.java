...
public class ServiceOrderStatusUseCaseTest {
    ...
    @Test
    void testTransitionToFinalizadaWithUnfinishedTasks_ShouldFail() {
        // Arrange
        ServiceOrder order = ServiceOrder.create(
                vehicleId,
                customerId,
        ...
        OrderTask unfinishedTask = OrderTask.create(orderId, UUID.randomUUID());

        when(serviceOrderRepository.findById(orderId)).thenReturn(Optional.of(order));
        when(orderTaskRepository.findByServiceOrderId(orderId)).thenReturn(List.of(unfinishedTask));

        // Act & Assert
        assertThrows(IllegalStateException.class,
                () -> useCase.updateStatus(orderId, "Finalizado", "Mecânico"));
    }

    @Test
    void testTransitionToFinalizadaWithAllTasksFinished() {
        // Arrange
        ServiceOrder order = ServiceOrder.create(
                vehicleId,
                customerId,
        ...
        OrderTask finishedTask = OrderTask.create(orderId, UUID.randomUUID());
        finishedTask.start();
        finishedTask.finish();

        when(serviceOrderRepository.findById(orderId)).thenReturn(Optional.of(order));
        when(serviceOrderRepository.save(any())).thenReturn(order);
        when(orderTaskRepository.findByServiceOrderId(orderId)).thenReturn(List.of(finishedTask));
        when(mapper.toResponse(any())).thenReturn(new ServiceOrderResponse());
        ...
        // Assert
        assertEquals(OrderStatus.FINALIZADO, order.getStatus());
        assertNotNull(order.getCompletedAt());
    }

    @Test
    void testTransitionToCancelWithAllTasksFinished() {
        // Arrange
        ServiceOrder order = ServiceOrder.create(
                vehicleId,
                customerId,
        ...
        // Act
        useCase.updateStatus(orderId, "Cancelado", "Mecânico");
    ...
    // ===== TESTES DE CENÁRIO DE CANCELAMENTO =====

    @Test
    void testCancelOrderInCriadaStatus() {
        // Arrange
        ServiceOrder order = ServiceOrder.create(
                vehicleId,
                customerId,
    ...
    @Test
    void testCancelOrderInEmDiagnosticoStatus() {
        // Arrange
        ServiceOrder order = ServiceOrder.create(
                vehicleId,
                customerId,
    ...
    @Test
    void testCancelOrderInEmExecucaoStatus() {
        // Arrange
        ServiceOrder order = ServiceOrder.create(
                vehicleId,
                customerId,
    ...
    @Test
    void testCancelOrderRegistersUpdatedAtTimestamp() {
        // Arrange
        ServiceOrder order = ServiceOrder.create(
                vehicleId,
                customerId,
        ...
        assertTrue(order.getUpdatedAt().isBefore(afterCancel) || order.getUpdatedAt().isEqual(afterCancel));
    }
}
